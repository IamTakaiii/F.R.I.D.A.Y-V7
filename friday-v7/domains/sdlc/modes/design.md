...
Counsel + Feature List + Arch (if any) on enter. **`design.deep.md` at Capture** — earlier only if HTTP/data/async/security/scale or `ละเอียด`.

## Run

| Phase | Do | Stop until |
|---|---|---|
| Discover | Outcome · evidence (Unknown/Assumed) · one question · ≥2 options + pick · stress the pick | User names the lock in one line (not `โอเค`). Mismatch → stay here. Do not dump the whole column. |
